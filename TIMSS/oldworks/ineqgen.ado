	...
	foreach i of local year{;
		...
		if "`index'" == "idx1" {;
			foreach j of local clist {;
				disp "`j'm`i'_`k'";
				qui ineqdeco `k' if inlist(cntry , "`j'") & inlist(wave , `i') , by(`bygroup')
				qui putexcel 
					A1=("cntry") B1=("wave") C1=("variable") D1=("R2")
						D1=("ge0") E1=("be0") F1=("we0") 
					A`excnt'=("`j'")  B`excnt'=("`i'") C`excnt'=("`k'") D`excnt'=(e(r2))
						D`excnt'=(r(ge0)) E`excnt'=(r(between_ge0)) F`excnt'=(r(within_ge0)) 
		...
		if "`index'" == "idx3" {;
			...
			disp "`j'm`i'_`k'";
			reg `k' dsex cbok cedu if inlist(cntry, `"`j'"') & inlist(wave, `i')  [`weight' `exp'];
			predict yhat`i' if e(sample) , xb;
			predict uhat`i' if e(sample) , residual;
			local beff = e(rmse)^2;
			mat reg`j'`i' = e(b);
			local bsex = reg`j'`i'[1,1]; capture local bbok = reg`j'`i'[1,2]; capture local bedu = reg`j'`i'[1,3]; capture local coef = reg`j'`i'[1,4];
			
			capture			 sum dsex [`weight' `exp'] if inlist(cntry, `"`j'"') & inlist(wave, `i') , meanonly; local msex = r(sum_w);
			capture			 sum cbok [`weight' `exp'] if inlist(cntry, `"`j'"') & inlist(wave, `i') , meanonly; local mbok = r(sum_w);
			capture			 sum cedu [`weight' `exp'] if inlist(cntry, `"`j'"') & inlist(wave, `i') , meanonly; local medu = r(sum_w);
			
			qui disp "coef" `coef'  " bsex"`bsex' " msex"`msex' " bbok"`bbok' " mbok"`mbok'  " bedu"`bedu' " medu"`medu';
			replace  `k'_c = `coef' + `bsex'*`msex' + `bbok'*`mbok' + `bedu'*`medu' + uhat`i' if inlist(cntry, `"`j'"') & inlist(wave, `i');
			
			qui list `k'_c in 1/5;
			qui sum ``k'_c' if inlist(cntry, `"`j'"') & inlist(wave, `i') ; local vyhat = r(Var);
			qui sum `k' if inlist(cntry, `"`j'"') & inlist(wave, `i') ; local v`k' = r(Var);
			
			local index = 1 - `vyhat' /`v`k'';
			disp `index';
				qui putexcel 
					A1=("cntry") B1=("wave") C1=("variable") D1=("R2")
			//			D1=("ge0") E1=("be0") F1=("we0") 
...
capture program drop vardeco;
program define vardeco, byable(recall) rclass ;
	syntax [varname] [if] [in] [fw aw/] 
	,BYgroup(varname) [Range(numlist min=2 max=2) options];
bys id: egen ybar = mean(y);
gen yd = y ybar;
reg y;
reg yabr;
reg yd;
