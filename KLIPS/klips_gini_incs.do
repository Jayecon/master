...
forvalue i = 1998/2018 {
	di "`i' , 0"
	keep if head & age1865 & men & year == `i'
	qui ineqdeco incs1 [aw=wgt] 
	matrix `temp1'[1,1] = `i'
	matrix `temp1'[1,2] = -1
	matrix `temp1'[1,3] = 0
...
capture label value area varea

save klips_gini_incs0.dta , replace
...
forvalue i = 1998/2018 {
		foreach k of local alist {
			...
			keep if head  & age1865 & men & year == `i' & area == `k'
			qui ineqdeco incs1 [aw=wgt] 
			matrix `temp1'[1,1] = `i'
			matrix `temp1'[1,2] = -1
			matrix `temp1'[1,3] = `k'
...
save klips_gini_incs1.dta , replace
