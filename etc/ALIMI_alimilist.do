...
import excel `path'alimi/ALIMI_list_2023.xls , sheet("empty") firstrow allstring
    label data "대학알리미 학교 개황(20231214)"
    compress
    destring _all , replace
    rename 학교구분 uniclass
        label var uniclass "학교구분(ALIMI)"
    ...
    rename 관련법령 legalbase
        label var legal"관련법령"
        label define LEGAL 1 "개별설치령에 의한 국립학교" 2 "고등교육법" ///
        3 "국민 평생 직업능력 개발법" 4 "기타" 5 "특별법국립" 6 "특별법법인"
    ...
    rename 우편번호 zipcode
        label var zipcode "우편번호"
    drop 학교코드 학교명한자 학교명영문 영문주소 중문주소 학교홈페이지 총장명 ///
    학교대표번호 학교대표팩스번호
    drop if uniclass == 2 /*대학원 제외*/
compress
