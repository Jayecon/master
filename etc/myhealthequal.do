...
********************************************************************************
	*	ACI 
		...
		if "`m'" == "aci" {
			tempname intercept sumw cumw cumw_1 cumwr cumwr_1 rank temp sigma meanlhs lhs intercept rhs aci aci_se aci_ll aci_ul m mse mll mul temp2
			*Calculate measure
			if `bounded'==1 {
				qui summarize `estimate'
			...
			matrix `temp2'=e(b)    //SJ corretction??? 250709 16:16
			gen `aci'=`temp2'[1,1] //SJ corretction??? 250709 16:16
			scalar `m'=`aci'
			return scalar aci=`m'
...
********************************************************************************
	*	RCI
		...
		if "`m'" == "rci" {
			tempname intercept sumw cumw cumw_1 cumwr cumwr_1 rank temp sigma meanlhs lhs intercept rhs rci rci_se rci_ll rci_ul m mse mll mul temp2
			*Calculate measure
			if `bounded'==1 {
				qui summarize `estimate'
				if r(min)>=xmin & r(max)<=xmax {
			...
			matrix `temp2'=e(b)    //SJ corretction??? 250709 16:16
			gen `rci'=`temp2'[1,1] //SJ corretction??? 250709 16:16
			scalar `m'=`rci'
			return scalar rci=`m'
			*Calculate 95% confidence intervals
			matrix V=e(V)
...
********************************************************************************
	*	RII
		...
		if "`m'" == "rii" {
			tempname sc estimate_sc sumw cumw cumw_1 cumwr cumwr_1 rank rii lnRII rii_var rii_se rii_ll rii_ul m mse mll mul temp2 temp3 temp4
			*Calculate measure
			qui summarize `estimate'
			gen `sc'=1 if r(max)<=1
			...
			if r(r)==1 {
                ...
                matrix `temp2'=e(b)    //SJ corretction??? 250709 16:16
                gen `rii'=`temp2'[1,1] //SJ corretction??? 250709 16:16
				scalar `m'=`rii'
				return scalar rii=`m'
				*Calculate 95% confidence intervals
				if "`svy'"=="" {
                ...
                matrix `temp3'=e(b)    //SJ corretction??? 250709 16:16
                gen `lnRII'=`temp2'[1,1] //SJ corretction??? 250709 16:16
                matrix `temp4'=e(V)    //SJ corretction??? 250709 16:16
                gen `rii_var'=`temp2'[1,1] //SJ corretction??? 250709 16:16
...
********************************************************************************
	*	SII
		...
		if "`m'" == "sii" {
			tempname sc estimate_sc sumw cumw cumw_1 cumwr cumwr_1 rank sii sii_se sii_ll sii_ul m mse mll mul temp2
			*Calculate measure
			qui summarize `estimate'
			gen `sc'=1 if r(max)<=1
				replace `sc'=100 if r(max)>1 & r(max)<=100
			...
			if r(r)==1 {
                ...
                matrix `temp2'=e(b)    //SJ corretction??? 250709 16:16
                gen `sii'=`temp2'[1,1] //SJ corretction??? 250709 16:16
				*Calculate 95% confidence intervals
				matrix V=e(V)
