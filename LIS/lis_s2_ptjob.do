    ...
    foreach k of local datalist {
        ...
        /*소득분위별 위험도 평균 계산*/
            ...
            forvalue i = 1/100 {
                summarize aptjob  [aw=hpopwgt] if pcgroup == `i' , meanonly
                local aptjobp`i' = r(mean)
                summarize xptjob  [aw=hpopwgt] if pcgroup == `i' , meanonly
        ...
        /*결과 출력*/
            ...
            local iso3 = iso3[1]
            if "`k'" == "fr" {
                di as text "cname,iso2,iso3,group,grtype,aptjob,xptjob"
            }
            forvalue i = 1/10 {
                di as text "`cname',`iso2',`iso3',`i',1,`aptjobd`i'',`xptjobd`i''"
            }
            forvalue i = 1/100 {
                di as text "`cname',`iso2',`iso3',`i',2,`aptjobp`i'',`xptjobp`i''"
            }
    }
