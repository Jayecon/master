...
use ~/dropbox/goms/goms.dta if inrange(f001 ,2000 ,2011), clear
...
forvalue j = 3/5 {
	...
	bys f001: eopcal income [aw=wgt] if sjob, env(edugrp) dom  value(150, 400)
	log close

	log using goms_domt_incgrp_income , replace
	bys f001: eopcal income [aw=wgt] if sjob , env(incgrp) dom value(150, 400)
	log close

	log using goms_domt_rgngrp_income , replace
	bys f001: eopcal income [aw=wgt] if sjob , env(rgngrp) dom value(150, 400)
	log close

	log using goms_domt_pcagrp_income , replace
	bys f001: eopcal income [aw=wgt] if sjob , env(pcagrp) dom value(150, 400)
	log close
