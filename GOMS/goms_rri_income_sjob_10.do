...
local achvlist income

set more off
set matsize 10000
cd ~/dropbox/goms/results
use ~/dropbox/goms/goms if inrange(f001 , 2000 , 2011) & sjob, clear
...
foreach k of local grplist {
	...
	eopcal `j' [aw=wgt] if f001 == `i' , env(`k') `rri' b(100)
	matrix `temp1'[1,1] = 2			 
	matrix `temp1'[1,2] = 3          
	matrix `temp1'[1,3] = `i'        
	...
	matrix `temp1'[1,8] = r(sd)   
	matrix `temp1'[1,9] = r(l95)     
	matrix `temp1'[1,10] = r(u95)     
	matrix `temp1'[1,11] = r(l90)     
	matrix `temp1'[1,12] = r(u90)     
	matrix `temp1'[1,13] = r(l99)     
	matrix `temp1'[1,14] = r(u99)     
	matrix `temp1'[1,15] = `scoretype'
	matrix `temp1'[1,16] = -1
	matrix `temp1'[1,17] = -1
...
foreach k of local grplist {
	...
	eopcal `j' [aw=wgt] if f001 == `i' & sex == `m'  , env(`k') `rri' b(100)
	matrix `temp1'[1,1] = 2			 
	matrix `temp1'[1,2] = 3          
	matrix `temp1'[1,3] = `i'        
	matrix `temp1'[1,4] = `grouptype'        
...
foreach k of local grplist {
	...
	eopcal `j' [aw=wgt] if f001 == `i' & f013 == `l'  , env(`k') `rri' b(100)
	matrix `temp1'[1,1] = 2			 
	matrix `temp1'[1,2] = 3          
	matrix `temp1'[1,3] = `i'        
	matrix `temp1'[1,4] = `grouptype'        
...
foreach k of local grplist {
	...
	eopcal `j' [aw=wgt] if f001 == `i' & hscgrp == `p'  , env(`k') `rri' b(100)
	matrix `temp1'[1,1] = 2			 
	matrix `temp1'[1,2] = 3          
	matrix `temp1'[1,3] = `i'        
	matrix `temp1'[1,4] = `grouptype'        
...
foreach k of local grplist {
	...
	eopcal `j' [aw=wgt] if f001 == `i' & majgrp == `q'  , env(`k') `rri' b(100)
	matrix `temp1'[1,1] = 2			 
	matrix `temp1'[1,2] = 3          
	matrix `temp1'[1,3] = `i'        
	matrix `temp1'[1,4] = `grouptype'        
...
matrix colname `temp2' = indextype wavetype wave envtype enttype sex index sd lc95 uc95 lc90 uc90 lc99 uc99 scoretype hstype majtype 
...
svmat `temp2' , names(matcol)
	...
	label var envtype "환경유형"
		label define ENVTYP 1 "부모학력환경(년도조정)" 2 "부모소득환경(년도조정)" 3 "지역환경" 4 "PCA(학력, 소득)" ///
			5 "PCA(학력, 소득, 지역)" 6 "PCA(학력, 소득, 성별)" 7 "부모학력환경" 8 "부모소득환경" 
...
save goms_rri_income_sjob_10.dta , replace
