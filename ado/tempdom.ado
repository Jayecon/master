...
while (`i' < `typnum') { ;
	...
	forvalues j= `iplus'/`typnum'{ ;
		...
		while !(`z' > `max`i'`j'') {;
			...
			matrix colname `temp' = S-order Group1 Group2 Z-value D1 D2 T over005 over025 over05 below95 below975 below995 ;
			...
			matrix `temp'[1,10] = `o05' ;	matrix `temp'[1,11] = `b95' ;	matrix `temp'[1,12] = `b975' ;
			matrix `temp'[1,13] = `b995' ;
