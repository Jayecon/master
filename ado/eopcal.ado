   ...
   [ENVironment(varname numeric )
    ...
    /* SUB PROGRAMS {{{*/
        ...
        /* ITT PROGRAM {{{*/
          ...
          program define itt , rclass sortpreserve ;
          syntax varlist [fw aw ] , ENVironment(str) EFTlv(integer) ;
          /// args : eopdist timew IOPAversion eftlv environment
          tempvar type nofg eftgrp expost gmean typmean tottypedepvar propo tottypecompen antepr temptime etwgt ltwgt
          ...
          /* Generate Ex-post Eop Achievement{{{*/
            bys `type' : gen `nofg' = _n ;
            foreach i of local typlist {;
              xtile _temp`i' = `nofg' if `type' == `i' , n(`eftlv') ;
            };
            pause ;
            ...
            gen `expost' = . ;
            forvalue i = 1/`eftlv' { ;
              sum `depvar' [`weight' `exp'] if `eftgrp' == `i' , meanonly ;
              replace `expost' = r(mean) if `eftgrp' == `i' ;
            };
          /*}}}*/
          /* Generate Ex-ante Propotional Eop Achievement {{{*/
            sum `depvar' [`weight' `exp'] , meanonly ;
            gen `gmean' = r(mean) ;
            gen `typmean' = . ;
            foreach i of local typlist { ;
              sum `depvar' [`weight' `exp'] if `type' == `i' , meanonly ;
              replace `typmean' = r(mean) if `type' == `i' ;
            };
            bys `type' : egen `tottypedepvar' = total( `depvar' * `wgt') ;
            gen `propo' =(`depvar' * `wgt' ) / `tottypedepvar' ;
            bys `type' : egen `tottypecompen' = total((`gmean' - `typmean') * `wgt') ;
            gen `antepr' =`depvar' +( `tottypecompen' * `propo') ;
            /*}}}*/
          /*Generate the time weight {{{*/
            xtset ;
            local id = r(panelvar) ;
            ...
            gen `etwgt' = sqrt(`tmax' - `temptime'  +1)/`numearly' ;
            local numlate = 0;
            forvalue i = 1/`tmax' {;
          ...
          /*Calculate ex-post, equal time and 1 aversion index */
            ...
            bys `id' : egen `idtotepdt' = total(`epdt' / `tmax');
          ...
          /*Calculate ex-ante pr, equal time and 1 aversion index */
            ...
            bys `id' : egen `idtoteadt' = total(`eadt' / `tmax');
          ...
          /*Calculate ex-post, early time and 1 aversion index */
            bys `id' : egen `idtotepdtearly' = total(`epdt' * `etwgt'  / `tmax');
          ...
          /*Calculate ex-ante pr, early time and 1 aversion index */
            bys `id' : egen `idtoteadtearly' = total(`eadt' * `etwgt'  / `tmax');
          ...
          /*Calculate ex-post, late time and 1 aversion index */
            bys `id' : egen `idtotepdtlate' = total(`epdt' * `ltwgt'  / `tmax');
          ...
          /*Calculate ex-ante pr, late time and 1 aversion index */
            bys `id' : egen `idtoteadtlate' = total(`eadt' * `ltwgt'  / `tmax');
