...
\begin{frame}
    ...
    \centering
    \begin{figure}
        \includegraphics[width=.4\textwidth]{pic/혁신도시.png}
        \\
        \raggedright
        \hspace{1em}
...
\begin{frame}[allowframebreaks]
    ...
    \framebreak%
        ...
        \end{equation}
        \begin{itemize}[<+->]
            \item 처치 집단: 서울 소재 대학을 순위 기준으로 두 가지로 범주화 : 1–20위와 21–50위.
    ...
    \framebreak%
        \item 삼중차분법을 이용하여 직무불일치의 임금불이익에 대한 정책의 영향을 추정. 
            \begin{align}
                ...
                &\quad + \beta_{4}\text{Mismatch}_{i,j,p,t} \times \text{KNU9}_{j}
                + \beta_{5}\text{Mismatch}_{i,j,p,t} \times D_{p,t}   \\ \nonumber
                &\quad + \beta_{6}D_{p,t} \times \text{KNU9}_{j}
                + X_{i,j,p,t}\gamma + \theta_{p} + \tau_{t} + \varepsilon_{i,j,p,t}.
            \end{align}
    \end{enumerate}
\end{frame}
...
\begin{frame}
    ...
    \end{table}
    \begin{itemize}[<+->]
        \item 비수도권은 대졸자는 수도권 대졸자에 비해 10.9\% 낮은 초임을 획득.
    \end{itemize}
\end{frame}

\begin{frame}
    \frametitle{초임격차: 대학순위·대학유형 분해}
    \begin{table}[ht]
        \centering
        \tiny
        \begin{tabular}{lcccc}
    ...
    \end{table}
    \begin{itemize}[<+->]
        \item 초임격차는 대학 순위에 의해 더욱 커짐.
        \item 초임격차는 지거국, 지방국공립대, 지방사립대 순서로 확대됨.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \end{table}
    \begin{itemize}[<+->]
        \item 지방대졸자가 지원가능한 정책대상 공공기관이 10개 증가할때마다 2-3\%의 초임상승 효과가 존재.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \end{table}
    \begin{itemize}[<+->]
        \item 정책효과는 공공기관에 직접 고용되지 않는 지방국공립대 및 지방사랍대 졸업자에게도 긍정적 효과.
        \item 이는 전이효과(spillover effect)로 해석할 수 있음.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \end{table}
    \begin{itemize}[<+->]
        \item 정책은 직무에 대한 과잉기술(overskill)을 경감.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \end{table}
    \begin{itemize}[<+->]
        \item 과잉기술 해소에 대한 정책효과는 인문계 졸업자에게서만 관찰.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \end{table}
    \begin{itemize}[<+->]
        \item 과잉기술 해소에 대한 정책효과는 여성 졸업자에게서만 관찰.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{table}[ht]
        ...
        수도권 소재 기준대학의 순위 & 1--20     & 21--50  \\
        \midrule                                                                                  
        $\text{초과숙련}_{ijpt} \times$ 지거국 $\times D_{pt}$      & -0.042  & -0.045   \\
                                                  & (0.030)   & (0.030)   \\
        $\text{초과숙련}_{ijpt} \times$ 지거국 & -0.007  & 0.036   \\
                                                  & (0.027)   & (0.033)   \\
        $\text{초과숙련}_{ijpt}$                        & -0.146*** & -0.184*** \\
                                                  & (0.015)   & (0.024)   \\
        \bottomrule
        \end{tabular}
    \end{table}
    \begin{itemize}[<+->]
        \item 정책효과는 임금불이익의 구조를 바꾸지는 못함.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{table}[ht]
        ...
        수도권 소재 기준대학의 순위    & 1--20     & 21--50    & 1--20     & 21--50    \\
        \midrule                                                          
        $\text{초과숙련}_{ijpt} \times$ 지거국 $\times D_{pt}$   & 0.015  & 0.016    & -0.104***  & -0.105*** \\
                                                  & (0.045)   & (0.045)   & (0.037)   & (0.037)   \\
        $\text{초과숙련}_{ijpt} \times$ 지거국 & -0.081**  & -0.073    & 0.007  & 0.109** \\
                                                  & (0.038)   & (0.048)   & (0.036)   & (0.044)   \\
        $\text{초과숙련}_{ijpt}$ & -0.165*** & -0.170*** & -0.068** & -0.167*** \\
                                                  & (0.019)   & (0.035)   & (0.022)   & (0.034)   \\
        \bottomrule
        \end{tabular}
    \end{table}
    \begin{itemize}[<+->]
        \item 정책효과는 자연계열 졸업자의 임금불이익의 구조를 오히려 심화.
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \begin{table}[ht]
        ...
        수도권 소재 기준대학의 순위    & 1--20     & 21--50    & 1--20     & 21--50    \\
        \midrule                                                          
        $\text{초과숙련}_{ijpt} \times$ 지거국 $\times D_{pt}$   & -0.040 & -0.046    & -0.046  & -0.052 \\
                                                  & (0.048)   & (0.048)   & (0.039)   & (0.039)   \\
        $\text{초과숙련}_{ijpt} \times$ 지거국    & -0.007  & 0.073    & -0.017  & -0.004 \\
                                                  & (0.042)   & (0.050)   & (0.035)   & (0.041)   \\
        $\text{초과숙련}_{ijpt}$ & 0.169*** & 0.238*** & -0.114*** & -0.120*** \\
                                                  & (0.022)   & (0.034)   & (0.020)   & (0.030)   \\
        \bottomrule
        \end{tabular}
    \end{table}
    \begin{itemize}[<+->]
        \item 정책은 성별에 따른 임금불이익의 구조에는 영향없음.
    \end{itemize}
\end{frame}
